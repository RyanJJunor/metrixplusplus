...
class Plugin(mpp.api.Plugin,
    ...
    def declare_configuration(self, parser):
        parser.add_option("--myext.operands_total", "--and_t",
                          action="store_true", default=False,
                          help="Enables collection of operators numbers metric")

    def configure(self, options):
        self.is_active_numbers = options.__dict__['myext.operands_total']

    def initialize(self):

        pattern_to_search = re.compile(
            '\sdelete\s|\snew\s|--|<<=|<<|<=|<|>>=|>>|>=|->\*|->|>|::|\+\+|\+=|\+|==|!=|\^=|\|=|\?[^:]+:|&&|&=|&|\*=|%=|/=|-=|=|-|\.\*|\*|/|\)|]|%|\.|\|\||\||,|~|\^|!|bool|char|double|float|int|long|short|signed|unsigned|void')
        # declare metric rules
        self.declare_metric(
            self.is_active_numbers,  # to count if active in callback
            self.Field('and_t', int),  # field name and type in the database    a + b + c
            # TODO metric regex
    ...
    def count_if_active(self, namespace, field, data, alias='*'):
        ...
        if field_data[0]._regions_supported == True:
            ...
            with open('C:/Users/ryanj/OneDrive - University of Stirling/Honours Project/Honours_Project/operators_unique.csv', 'r') as read_obj:
                with open('C:/Users/ryanj/OneDrive - University of Stirling/Honours Project/Honours_Project/operands_total.csv', 'wb') as write_obj:
    ...
    class Counter(mpp.api.MetricPluginMixin.IterIncrementCounter):
        ...
        def increment(self, match):
            ...
            if name not in Plugin.functions:
                self.func_metric = {match.group(): operand_calc(match.group())}
